...
class IPBMFZ_Post_Exporter
{
  ...
  public function export_post($post_id, $options = array())
  {
    ...
    try {
      ...
      return new WP_Error(
        'export_exception',
        sprintf(__('エクスポート中にエラーが発生しました: %s', 'wp-single-post-migrator'), $e->getMessage())
      );
    }
  }
  ...
  private function generate_post_xml($post, $temp_dir, $options)
  {
    ...
    if (file_put_contents($xml_path, $xml_content) === false) {
      return new WP_Error(
        'xml_write_failed',
        __('XMLファイルの書き込みに失敗しました。', 'wp-single-post-migrator')
      );
    }
  ...
  private function collect_post_images($post, $temp_dir)
  {
    ...
    if (!wp_mkdir_p($images_dir)) {
      return new WP_Error(
        'images_dir_failed',
        __('画像ディレクトリの作成に失敗しました。', 'wp-single-post-migrator')
      );
    }
  ...
  private function copy_image_to_export($image_url, $images_dir)
  {
    ...
    if (strpos($image_url, $upload_url) !== 0) {
      return new WP_Error(
        'external_image',
        sprintf(__('外部画像はエクスポートできません: %s', 'wp-single-post-migrator'), $image_url)
      );
    }
    ...
    if (!file_exists($source_path)) {
      return new WP_Error(
        'image_not_found',
        sprintf(__('画像ファイルが見つかりません: %s', 'wp-single-post-migrator'), $source_path)
      );
    }
    ...
    if (!copy($source_path, $dest_path)) {
      return new WP_Error(
        'copy_failed',
        sprintf(__('画像のコピーに失敗しました: %s', 'wp-single-post-migrator'), $filename)
      );
    }
  ...
  private function create_export_zip($temp_dir, $post)
  {
    ...
    if ($result !== TRUE) {
      return new WP_Error(
        'zip_creation_failed',
        sprintf(__('ZIPファイルの作成に失敗しました: %s', 'wp-single-post-migrator'), $result)
      );
    }
    ...
    if (!file_exists($zip_path)) {
      return new WP_Error(
        'zip_not_created',
        __('ZIPファイルが作成されませんでした。', 'wp-single-post-migrator')
      );
    }
