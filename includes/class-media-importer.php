...
class IPBMFZ_Media_Importer
{
  ...
  public function import_image($file_path, $post_id, $filename)
  {
    ...
    if (!file_exists($file_path)) {
      return new WP_Error(
        'file_not_found',
        __('ファイルが見つかりません: ' . $filename, 'wp-single-post-migrator')
      );
    }
  ...
  private function create_new_attachment($file_path, $post_id, $filename)
  {
    ...
    if (!$file_info['type']) {
      return new WP_Error(
        'invalid_file_type',
        __('サポートされていないファイル形式です: ' . $filename, 'wp-single-post-migrator')
      );
    }
    ...
    if (!copy($file_path, $new_file_path)) {
      return new WP_Error(
        'file_copy_failed',
        __('ファイルのコピーに失敗しました: ' . $filename, 'wp-single-post-migrator')
      );
    }
  ...
  public function validate_image_file($file_path, $filename)
  {
    ...
    if ($file_size > $max_size) {
      return new WP_Error(
        'file_too_large',
        sprintf(
          __('ファイルサイズが上限を超えています (%s): %s', 'wp-single-post-migrator'),
    ...
    if (strpos($file_info['type'], 'image/') !== 0) {
      return new WP_Error(
        'not_image',
        __('画像ファイルではありません: ' . $filename, 'wp-single-post-migrator')
      );
    }
