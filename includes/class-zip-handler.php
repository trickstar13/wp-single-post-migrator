...
class IPBMFZ_ZIP_Handler
{
  ...
  public function upload_and_extract($file_data, $post_id)
  {
    ...
    if (empty($image_files)) {
      ...
      return new WP_Error(
        'no_images_found',
        __('ZIPファイル内に対応する画像が見つかりませんでした。対応形式: ' . implode(', ', $this->supported_extensions), 'wp-single-post-migrator')
      );
    }
  ...
  private function validate_zip_file($file_data)
  {
    ...
    if ($file_data['error'] !== UPLOAD_ERR_OK) {
      switch ($file_data['error']) {
        ...
        case UPLOAD_ERR_FORM_SIZE:
          return new WP_Error(
            'file_too_large',
            __('ファイルサイズが上限を超えています。最大: ' . ini_get('upload_max_filesize'), 'wp-single-post-migrator')
          );
        case UPLOAD_ERR_PARTIAL:
          return new WP_Error(
            'upload_incomplete',
            __('ファイルのアップロードが不完全です。', 'wp-single-post-migrator')
          );
        case UPLOAD_ERR_NO_FILE:
          return new WP_Error(
            'no_file',
            __('ファイルが選択されていません。', 'wp-single-post-migrator')
          );
        default:
          return new WP_Error(
            'upload_error',
            __('ファイルのアップロードエラーが発生しました。', 'wp-single-post-migrator')
          );
      }
    }
    ...
    if ($file_info['ext'] !== 'zip') {
      return new WP_Error(
        'invalid_file_type',
        __('ZIPファイルのみアップロード可能です。', 'wp-single-post-migrator')
      );
    }
  ...
  private function create_temp_directory($post_id)
  {
    ...
    if (!wp_mkdir_p($temp_path)) {
      return new WP_Error(
        'temp_dir_failed',
        __('一時ディレクトリの作成に失敗しました。', 'wp-single-post-migrator')
      );
    }
  ...
  private function extract_zip($zip_path, $extract_path)
  {
    ...
    if ($result !== TRUE) {
      return new WP_Error(
        ...
        __('ZIPファイルの展開に失敗しました。ファイルが破損している可能性があります。', 'wp-single-post-migrator')
      );
    }
    ...
    if (!$extracted) {
      return new WP_Error(
        'zip_extraction_failed',
        __('ZIPファイルの展開に失敗しました。', 'wp-single-post-migrator')
      );
    }
