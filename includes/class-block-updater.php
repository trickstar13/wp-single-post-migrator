    ...
    public function update_blocks($post_id, $image_map) {
        ...
        if (!$validation['valid']) {
            return new WP_Error(
                ...
                __('ブロックの検証に失敗しました: ', 'import-post-block-media-from-zip') . implode(', ', $validation['errors'])
            );
        }
        ...
        if (empty($test_blocks) && !empty($blocks)) {
            return new WP_Error(
                ...
                __('コンテンツの再構築に失敗しました。', 'import-post-block-media-from-zip')
            );
        }
        ...
        if (is_wp_error($update_result) || !$update_result) {
            ...
            if ($db_result === false) {
                ...
                return new WP_Error('save_failed', __('記事の保存に失敗しました。', 'import-post-block-media-from-zip'));
            } else {
        ...
        foreach ($saved_blocks as $i => $saved_block) {
            if ($saved_block['blockName'] === 'core/image' && isset($saved_block['attrs']['id'])) {
                ...
                if (in_array($saved_id, $updated_ids)) {
                    if (strpos($saved_block['innerHTML'], 'wp-image-' . $saved_id) === false ||
    ...
    private function update_image_block(&$block, $image_map, &$results) {
        ...
        if (!$attachment_url) {
            $results['failed_matches'][] = sprintf(
                __('添付ファイルのURLを取得できませんでした: %s', 'import-post-block-media-from-zip'),
                $filename
            );
            return false;
        }
        ...
        if (strpos($block['innerHTML'], $attachment_url) === false ||
        ...
        foreach ($blocks as $block) {
            ...
            if (!isset($block['attrs']) || !isset($block['innerHTML'])) {
                $validation['errors'][] = sprintf(
                    __('ブロック構造が不正です: %s', 'import-post-block-media-from-zip'),
                    $block['blockName']
                );
            ...
            if ($block['blockName'] === 'core/image') {
                if (empty($block['attrs']['url']) && empty($block['innerHTML'])) {
                    $validation['warnings'][] = __('画像URLが見つからないブロックがあります', 'import-post-block-media-from-zip');
                }
            }
